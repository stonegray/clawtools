...
- [x] **26. Tool count "25" is wrong in three places — actual count is 23.**
...
- [x] **27. Tool-groups table lists `apply_patch` and `process` which are not in the catalog.**
...
- [x] **28. `ToolResultMessage.details` field documented but does not exist in the type.**
...
- [x] **29. Node.js minimum version contradicted between `package.json` and README.**
...
- [x] **30. Plugin entry-point resolution order documented in wrong sequence.**
...
- [x] **31. `expandGroups` JSDoc example includes `apply_patch` in `group:fs`.**
  `src/tools/catalog.ts` — the JSDoc example shows `"group:fs" → ["read","write","edit","apply_patch"]`;
  the actual expansion is `["read","write","edit"]`.

- [x] **32. `onFactoryError` callback has no `@param` JSDoc on any of the three methods that accept it.**
...
- [x] **33. Module-level JSDoc example calls `createTool()` without a `FileBridge`.**
  The example silently produces empty arrays for all fs tools with no indication to the reader
  that `FileBridge` is required.

- [x] **34. `loadCoreToolsSync` lacks a `@see` tag pointing to the async variant.**
  Users relying on IDE hover see no indication that an async option exists.

- [x] **35. `loadCoreToolsSync` JSDoc says factories are "registered lazily" — they are not.**
...
- [x] **36. `TODO.md` has completed tasks still marked unchecked.**
  "Add user tool registration" and "Write unit tests" are both implemented but remain as
  `[ ]` items in the file.

- [x] **37. Duplicated word in `docs/features.md`.**
  "and and an advanced custom bundler" → "and an advanced custom bundler".

- [x] **38. Typo in `docs/features.md`.**
  "resilliant" → "resilient".

- [x] **39. Example model ID inconsistency between `examples/agentic/index.ts` and README.**
...
- [x] **40. Existing `docs/review.md` incorrectly states "Zero runtime dependencies".**
  `@sinclair/typebox`, `ajv`, and `undici` are listed in `dependencies` (not `devDependencies`)
  and will be installed by consumers.

- [x] **41. Existing `docs/review.md` states `engines: ">=18"`; actual field is `">=20.0.0"`.**

---

## 🟡 Test Gaps

- [x] **42. `lazy: true` option on `loadCoreTools` has no test coverage.**
  No test verifies that `createTool()` returns `[]` before loading completes and is populated
  after the load resolves.

- [x] **43. `onFactoryError` positive path is untested.**
...
- [x] **44. `loadCoreToolsSync` core invariant (null-factory → empty `createTool()`) is untested.**
  The most important behavioral guarantee of the sync path has no direct assertion.

- [x] **45. Magic number `23` in discovery test has no explanatory comment.**
...
- [x] **46. `profile: "full"` wildcard behavior (bug #6) has zero test coverage.**
  No test verifies whether `"full"` tools appear or don't appear in non-`"full"` profile
  queries, so the bug can regress silently.

- [x] **47. No test re-registers a connector with the same ID but a different `api` transport.**
  The stale `apiIndex` bug (#2) is completely uncovered by the existing connector tests.

- [x] **48. No test re-registers a connector with the same ID but a different `provider` name.**
  The stale `providerIndex` bug (#3) is uncovered.

- [x] **49. `thinking_delta` / `thinking_end` streaming events have no test coverage.**
  No mock scenario in the invariants suite produces thinking-block events.

- [x] **50. Multi-tool-call streams are never tested.**
  Only single `toolcall_start…end` blocks are exercised; multiple sequential tool calls in
  one stream are not.

- [x] **51. Abort test does not verify the abort `reason` / `cause` is propagated.**
  The test asserts rejection but never checks that the cause value is the one passed to
  `AbortController.abort()`.

- [x] **52. `loadCoreTools` source-path fallback is never unit-tested in isolation.**
...
- [x] **53. `vitest.config.ts` `e2e` include pattern matches no files.**
...
- [x] **54. `moduleCache` and `loadedModules` in `catalog.ts` are ambiguously named.**
  Could be confused with Node's built-in module registry. `sourceModuleCache` /
  `bundleModuleCache` would be clearer.

- [x] **55. Destructured `_catalogFactory` in `loadCoreTools` is never used.**
  Should be omitted from the destructuring pattern entirely.

- [x] **56. Audit `dependencies` vs `devDependencies` for `@sinclair/typebox`, `ajv`, `undici`.**
...
- [x] **57. No `lint` script in `package.json` despite `eslint-disable` comments in source.**
  Either add a linter config + `"lint"` script, or remove the stale disable comments.

- [x] **58. `"verbatimModuleSyntax": false` in `tsconfig.json`.**
...
- [x] **59. Blank-string error message is identical to missing-parameter error in `params.ts`.**
...
- [x] **60. `"full"` overloads both a profile identifier and a wildcard flag.**
